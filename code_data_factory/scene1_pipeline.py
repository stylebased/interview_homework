...
"""
Scene 1: Generate code / business Q&A pairs from code chunks.

Flow:
1. Read code chunks from data/chunks.json (generated by analyzer).
2. For each chunk, ask the LLM to:
   - create realistic questions
   - produce a thinking_trace (step-by-step reasoning)
   - provide final answers
3. Save all results to data/scene1_raw.jsonl.

The actual SFT-style formatting is handled later by postprocess.py.
"""
